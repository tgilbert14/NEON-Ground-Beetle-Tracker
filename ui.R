            ...
            div(id = "envDredgeBlock", class = "env-dredge",
              div(class = "env-dredge-head",
                ...
                info_pop("Why this 'best driver' is probably not a real signal",
                  p("This panel hunts through dozens of driver-and-lag combinations and shows you the strongest match it finds. That is a fishing expedition, not a test, so treat the headline gently. Here is the honest read on the precipitation hit:"),
                  p(tags$b("The luck check did not pass."), " The match (r = +0.45) is the best of 52 driver-and-lag combinations over just 28 overlapping months. When you look 52 times, your best look is usually large by chance. The shuffle test agrees: random reshuffles beat this match about 79 times in 100 (p = 0.79). So this is most likely noise, which is why we label it Apparent, not real."),
                  p(tags$b("A 9-month lag does not fit beetle biology."), " What a pitfall trap counts is activity, how much beetles walk, not how many there are. Warm, wet nights get desert beetles moving the same night, so any real rain effect should show up fast, within a month or so, not three quarters of a year later. A 9-month delay has no carabid mechanism behind it. It is just the lag where two wandering lines happened to cross."),
                  p(tags$b("Monsoon storms wash the signal out, literally."), " Heavy rain floods and overflows the trap cups, so specimens float out and catch is lost. It also dilutes the preservative and can knock traps around. The rainiest months are exactly the months with the least reliable catch and the least certain effort, so a true short-lag rain effect gets buried by lost samples. And many desert ground beetles are spring or night active, only loosely tied to the peak summer rains in the first place."),
                  p(tags$b("Read this panel as a place to generate ideas, never as proof that rain drives beetle activity."), " The honest next step is a within-season look at the summer monsoon months, not a longer lag."))),
